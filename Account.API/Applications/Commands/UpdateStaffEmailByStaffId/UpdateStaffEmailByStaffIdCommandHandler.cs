...
public class UpdateStaffEmailByStaffIdCommandHandler(IUnitOfWork unitOfWork, AppService service) : IRequestHandler<UpdateStaffEmailByStaffIdCommand, Result>
{
    ...
    public async Task<Result> Handle(UpdateStaffEmailByStaffIdCommand request, CancellationToken cancellationToken)
    {
        try
        {
            ...
            var conflict = await _unitOfWork.Staffs.GetAnyByIdentitiesExcludingIdsAsNoTrackingAsync((IdentityParameter.Email, [(staff.Id, request.Email)])); 
            if (conflict)
            {
                return Result.Failure($"Email '{request.Email}' already registered", ResponseStatus.Conflict);
            }

            staff.UpdateEmail(request.Email);
