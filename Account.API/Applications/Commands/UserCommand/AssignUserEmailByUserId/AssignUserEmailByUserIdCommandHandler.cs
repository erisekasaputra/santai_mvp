...
public class AssignUserEmailByUserIdCommandHandler(
    IUnitOfWork unitOfWork,
    ApplicationService service,
    IEncryptionService kmsClient,
    IHashService hashService) : IRequestHandler<AssignUserEmailByUserIdCommand, Result>
{
    private readonly IUnitOfWork _unitOfWork = unitOfWork;
    private readonly ApplicationService _service = service;
    private readonly IEncryptionService _kmsClient = kmsClient;
    private readonly IHashService _hashService = hashService;
    public async Task<Result> Handle(AssignUserEmailByUserIdCommand request, CancellationToken cancellationToken)
    {
        try
        {
            ...
            var conflict = await _unitOfWork.BaseUsers.GetAnyByIdentitiesExcludingIdAsNoTrackingAsync(request.UserId, (IdentityParameter.Email, hashedEmail));
            if (conflict)
            {
