    ...
    public async Task<Result<IEnumerable<ItemSoldDto>>> Handle(AddItemSoldQuantityCommand request, CancellationToken cancellationToken)
    {
        var result = await _asyncRetryPolicy.ExecuteAsync(async () =>
        {
            try
            {
                ...
                var itemIds = request.AddItemSoldQuantityRequests.Select(x => x.ItemId).ToList();
                ...
                var missingItemRequests = request.AddItemSoldQuantityRequests.Where(x => !retrievedItemIds.Contains(x.ItemId)).ToList();
                ...
                {
                    var quantity = request.AddItemSoldQuantityRequests.First(x => x.ItemId == item.Id).Quantity; 
