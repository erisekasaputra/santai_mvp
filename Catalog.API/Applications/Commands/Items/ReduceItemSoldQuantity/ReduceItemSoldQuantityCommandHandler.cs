...
public class ReduceItemSoldQuantityCommandHandler : IRequestHandler<ReduceItemSoldQuantityCommand, Result<IEnumerable<ItemSoldDto>>>
{
    private readonly AsyncRetryPolicy _asyncRetryPolicy;
    private readonly IUnitOfWork _unitOfWork;
    public ReduceItemSoldQuantityCommandHandler(IUnitOfWork unitOfWork, ApplicationService service)
    {
    ...
    public async Task<Result<IEnumerable<ItemSoldDto>>> Handle(ReduceItemSoldQuantityCommand request, CancellationToken cancellationToken)
    {
        var result = await _asyncRetryPolicy.ExecuteAsync(async () =>
        {
            try
            {
                ...
                var itemIds = request.ReduceItemSoldQuantityRequests.Select(x => x.ItemId).ToList();
                ...
                var missingItemRequests = request.ReduceItemSoldQuantityRequests.Where(x => !retrievedItemIds.Contains(x.ItemId)).ToList();
                ...
                {
                    var quantity = request.ReduceItemSoldQuantityRequests.First(x => x.ItemId == item.Id).Quantity;

                    try
                    {
                        item.ReduceSoldQuantity(quantity);
