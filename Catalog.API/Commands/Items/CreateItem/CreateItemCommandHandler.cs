...
public class CreateItemCommandHandler(IUnitOfWork unitOfWork) : IRequestHandler<CreateItemCommand, Result<CreateItemResponse>>
{
    ...
    public async Task<Result<CreateItemResponse>> Handle(CreateItemCommand request, CancellationToken cancellationToken)
    {
        ...
        {
            return Result<CreateItemResponse>.Failure("Owner reviews can not be null, please fill at least one key value pair, e.g : [Performance: 9.2]");
        }
        ...
        var responseDto = new CreateItemResponse(
                                                    response.Id,
                                                    response.Name,
                                                    ...
                                                    response.Brand.Id,
                                                    response.Brand.Name,
                                                    response.OwnerReviews
                                                );

        return Result<CreateItemResponse>.SuccessResult(responseDto, [], 201);
    }
}
