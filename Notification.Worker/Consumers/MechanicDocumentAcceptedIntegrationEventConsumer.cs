...
public class MechanicDocumentAcceptedIntegrationEventConsumer(
    ...
    ILogger<MechanicDocumentAcceptedIntegrationEventConsumer> logger,
    NotificationDbContext dbContext,
    INotificationService notificationService) : IConsumer<MechanicDocumentAcceptedIntegrationEvent>
{
    private readonly INotificationService _notificationService = notificationService;
    private readonly NotificationDbContext _dbContext = dbContext;
    private readonly ILogger<MechanicDocumentAcceptedIntegrationEventConsumer> _logger = logger;
    ...
    public async Task Consume(ConsumeContext<MechanicDocumentAcceptedIntegrationEvent> context)
    {
        var orderData = context.Message;

        try
        {
            await _notificationService.SaveNotification(new Notify(orderData.MechanicId.ToString(), NotifyType.Information.ToString(), "Documents", $"Your documents have been approved by SANTAIMOTO. Congratulations! You are now ready to start receiving orders."));
        }
        catch (Exception ex)
        {
        ...
        foreach (var profile in target.Profiles)
        {
            var fcmPayload = new
            {
                ...
                {
                    title = "SANTAIMOTO",
                    body = $"Your documents have been approved by SANTAIMOTO. Congratulations! You are now ready to start receiving orders.",
